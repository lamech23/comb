  ...
  const handelSubmit = async(e) =>{
    ...
    try {
      ...
      if(house_name.length === 0){
        ...
        const response  = await axios.post("http://localhost:4000/houseRegister/houseName",{ house_name: house_name, user_id: user_id})
        if(response){
          setErrors(null)
          setHouse_name("")
...
<div class="mb-6">
  ...
  <label for="username-success" class="block mb-2 text-sm font-medium text-green-700 dark:text-green-500">Please  Enter Your House Name</label>
  <input 
