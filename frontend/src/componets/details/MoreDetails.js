...
const handelSubmit = async( e) =>{
     ...
     try {
      ...
      if(first_name===''||second_name===''||phoneNumber===''||id_number===''||postal_address===''||gender===''){
...
const handelSelect = async(e)=>{
  ...
  if(selectedDate==='' || time===''){
    ...
    const response= await axios.post('http://localhost:4000/ClientTour/tour',{
...
formData.append('category', category)
    const fetchDetails = async () => {
      const response = await axios.get(`http://localhost:4000/RelatedHouses/?category=${category}` ,formData)
...
<p className=' fs-4'id='ctg'><strong className='text-white fs-1 ' > {category}</strong></p>
</div>
...
<div className=' mt-5' style={{padding:'20px  0 '}}>
      <div className='row  d-flex  ms-5 mt-4'>
          <div   className='  col-lg-8 col-md-6  ms-2 mb-2   ' style={{width:'350px' }}>
            
            <img className='  ms-5 mt-5 mb-3 ' src={`http://localhost:4000/${image}`} width="350px" height="350px" style={{borderRadius:'20px'}} alt="" />
          </div>
          ...
          <p className=' fs-4'> House Category<strong> {category}</strong></p>
          <p className=' fs-4'>  <strong> Features  {title}</strong></p>
                {/* <p className=' fs-4 ' > <strong>{contact}</strong></p> */}
            <p className=' text-lead ms-4'> <strong>{description}</strong></p>

                <p className=' fs-3'> <strong className='text-danger'> Ksh: {price}</strong></p>
         ...
         <div className='third_card '>
            <a data-bs-toggle="modal" href="#contactAg" role="button" className="request">
              Contact Agent 
            </a >
            < a data-bs-toggle="modal" href="#tour" role="button" className="request mt-5">
            Request A tour
            ...
            <div className='text-center'>
          <div className="modal fade" id="contactAg" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header ">
              <h1 className="modal-title fs-3  " id="exampleModalToggleLabel">Contact Agent  </h1>
      ...
      {user?<button type='submit' className="btn btn-outline-warning w-75 btn-lg mt-2 ms-5 mb-3" data-bs-dismiss="modal">submit</button>:<div type='submit' className="btn btn-outline-warning w-75 btn-lg mt-2 ms-5 mb-3"data-bs-dismiss="modal" onClick={prevent}>submit</div>}
                  ...
                  {/* form-2 */}
                  <div className='text-center'>
          <div className="modal fade" id="tour" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header ">
              <h1 className="modal-title fs-3  " id="exampleModalToggleLabel">Tour with a Buyer's Agent  </h1>
              <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div className="modal-body text-start ">
...
{      user?<button type='submit' className="btn btn-outline-warning w-75 btn-lg mt-2 ms-5 mb-3" data-bs-dismiss="modal">submit</button>:<div type='submit' className="btn btn-outline-warning w-75 btn-lg mt-2 ms-5 mb-3"data-bs-dismiss="modal" onClick={prevent}>submit</div>
            ...
            { details.map((detail)=>(
              ...
              <div  key={detail.id} className='  col-lg-8 col-md-6  ms-2 mb-2  justify-content-between card shadow-lg ' style={{width:'350px' }} id="hover">
                <Link to={`/MoreDetails/${detail.id}`}>
                <img className='  ms-5 mt-5 mb-3' src={`http://localhost:4000/${detail.image}`} width="250px" height="250px" style={{borderRadius:'20px'}} alt="" />
                </Link>
            ...
            <strong>{detail.description}</strong>
              ...
              <strong>{detail.price}</strong>
                    ...
                    <Link className='text-decoration-none ' to={`/DetailsInfo/${detail.id}`}>See more...</Link>
            </div>
                    <p className='mt-2 fs-5'><strong>{formatDistanceToNow(new Date(detail.createdAt),{addSuffix:true})}</strong></p>
                
                  <Link to={`/DetailsInfo/${detail.id}`}> <button className="btn btn-outline-secondary btn-lg mt-2 ms-5 mb-3">Buy Now</button></Link>
